...
def prepare_data():
    ...
    for index in range(script_args.num_collect_files):
        with open(f'data/{script_args.model_prefix}/data_{script_args.iter}/stage_2_allOutputs_{index}.json', 'r') as f:
            data = json.load(f)
    ...
    for i, item in enumerate(collected_data):
        if item['outputs'] == []:
            # no chosen
            continue
        if len(item['outputs']) == len(all_data[i]):
            # no rejected
            continue
        ...
        margin.append(script_args.margin_scale)
...
if __name__ == "__main__":
    ...
    train_test_dataset = dataset.train_test_split(test_size=0.01, seed=42)
